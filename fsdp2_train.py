...
def save_checkpoint(
    ...
    model_state_dict = get_model_state_dict(model, options=state_dict_options)
...
def shard_gpt_model(model: GPT, mp_policy: MixedPrecisionPolicy) -> GPT:
...
class Trainer:
    def __init__(
        ...
        mp_policy = MixedPrecisionPolicy(
            param_dtype=pt_dtype,
            reduce_dtype=torch.float32,
        )

        self.model = model.to(device=self.device)
        self.model = shard_gpt_model(self.model, mp_policy=mp_policy)
    ...
    def _load_checkpoint(self) -> None:
        ...
        sync_objects = [scheduler_state_dict, metadata]
        dist.broadcast_object_list(sync_objects, src=0)

        scheduler_state_dict, metadata = sync_objects
        self.scheduler.load_state_dict(scheduler_state_dict)
        self.train_losses = metadata["train_losses"]
        self.valid_losses = metadata["valid_losses"]
        self.completed_steps = metadata["completed_steps"]
        self.best_valid_loss = metadata["best_valid_loss"]
    ...
    def train(self) -> None:
        ...
        if self.is_main_process:
            print("Training GPT2 over the dataset once with FSDP2")
...
def main():
    ...
    if trainer.is_main_process:
        ...
        print(f"Using FSDP2 mixed precision with param_dtype={pt_dtype} and reduce_dtype=torch.float32")
